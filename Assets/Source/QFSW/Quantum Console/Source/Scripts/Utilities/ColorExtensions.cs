...
namespace QFSW.QC.Utilities
{
    public static class ColorExtensions
    {
        ...
        {
            string HexR = hex[1].ToString() + hex[2].ToString();
            string HexG = hex[3].ToString() + hex[4].ToString();
            string HexB = hex[5].ToString() + hex[6].ToString();

            float r = (float)int.Parse(HexR, System.Globalization.NumberStyles.HexNumber) / 255.0f;
            float g = (float)int.Parse(HexG, System.Globalization.NumberStyles.HexNumber) / 255.0f;
            float b = (float)int.Parse(HexB, System.Globalization.NumberStyles.HexNumber) / 255.0f;
